...
export default class Menu extends React.Component{
    ...
    render(){
        return <View  style = {this.props.styles["safeAreaView"]}>
            {this.state.ifOpened === false ? <FlatList
                ...
                renderItem = {({item, index}) => <Pressable style = {this.props.styles["menuOption"]} onPress = {() => {this.openTheCar(item);}} key = {index}>
                    <Text style = {this.props.styles["menuOptionText"]}>{item}</Text>
                </Pressable>}
            /> : <Main styles = {this.props.stylesForMain} onGoingBack = {() => {this.goBack()}} openedcar = {this.state.currentCar} data = {this.listData}/>}
        </View>;
    }
}